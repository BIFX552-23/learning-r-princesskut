...
```{r}
ggplot(data = gapminder, mapping = aes(x = year, y = lifeExp, col = "continent")) + geom_point()
```
...
The lines are now on top of the lines not through the lines.
...
ggplot(data = gapminder, mapping = aes(x = gdpPercap, y = lifeExp, col = "continent")) +
  geom_point(size = 2, shape = 17, col = "black") + 
  scale_x_log10() +
  geom_smooth(method="lm", linewidth=1.5)
```
...
```{r}
# your code goes here
(g1 <-ggplot(gapminder, aes(x = continent, y = lifeExp)) + geom_boxplot())
g1 + ylab('Life Expectancy') + xlab(NULL) +theme(axis.text.x=element_blank())
```
